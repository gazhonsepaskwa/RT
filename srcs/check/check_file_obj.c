...
int check_sp(char **split, char *line, int l)
{
	...
	if (tab_len(split) < 8 || tab_len(split) > 9)
		error += ac_error(l, line, "sp (x,y,z)[position] (int)[radius] (rgb)[color] (float)[ambiant] (float)[difuse] (float)[specular] (float)[exponent]", (int[2]){9, tab_len(split)});
	error += format_vec(split[1], l, line, 1);
	...
	if (ft_atof(split[4]) > 1.0 || ft_atof(split[4]) < 0.0)
		error += raise_error(l, "Ambiant composant should be in range [0.0-1.0]", line, 4);
	if (ft_atof(split[5]) > 1.0 || ft_atof(split[4]) < 0.0)
		error += raise_error(l, "Difuse composant should be in range [0.0-1.0]", line, 5);
	if (ft_atof(split[6]) > 1.0 || ft_atof(split[4]) < 0.0)
		error += raise_error(l, "Specular composant should be in range [0.0-1.0]", line, 6);
	return (error);
}
...
int check_pl(char **split, char *line, int l)
{
	...
	error = 0;
	if (tab_len(split) < 8 || tab_len(split) > 9)
		error += ac_error(l, line, "pl (x,y,z)[position] (x,y,z)[normal] (rgb)[color] (float)[ambiant] (float)[difuse] (float)[specular] (float)[exponent]", (int[2]){9, tab_len(split)});
	error += format_vec(split[1], l, line, 1);
	error += format_vec(split[2], l, line, 2);
	error += format_rgb(split[3], l, line, 3);
	if (ft_atof(split[4]) > 1.0 || ft_atof(split[4]) < 0.0)
